...
int8_t ProtoNNF::getInitErrorCode(){
	...
	if ((d !=protoNNParam::featDim))
		this->errorCode |= 1;

    if (d_cap !=protoNNParam::ldDim)
    	this->errorCode |= 2;

    if (m !=protoNNParam::numPrototypes)
    	this->errorCode |= 4;

    if(L !=protoNNParam::numLabels)
		this->errorCode |= 8;

	if ((gamma - (protoNNParam::gamma)) >= 0.001 ||
		(gamma - (protoNNParam::gamma)) <= -0.001){
		this->errorCode |= 16;
	}
 ...
 */
ProtoNNF::ProtoNNF() {
	this->featDim =protoNNParam::featDim;
	this->ldDim =protoNNParam::ldDim;
	this->numPrototypes =protoNNParam::numPrototypes;
	this->numLabels =protoNNParam::numLabels;
	this->gamma = protoNNParam::gamma;
...
int8_t ProtoNNF::getPrototype(unsigned index, float *prototype){
	...
	for(unsigned i = 0; i < d_cap; i++){
		float component = protoNNParam::prototypeMatrix[index * d_cap + i];
...
int8_t ProtoNNF::getPrototypeLabel(unsigned index, float *prototypeLabel){
	...
	for(unsigned i = 0; i < L; i++){
		float component = protoNNParam::prototypeLabelMatrix[index * L + i];
...
float ProtoNNF::getProjectionComponent(unsigned i, unsigned j){
	unsigned int d = this->featDim;
	return protoNNParam::ldProjectionMatrix[i * d + j];
}
