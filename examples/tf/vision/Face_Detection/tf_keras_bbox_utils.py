...
def intersect(box_a, box_b):
    ...
    A = box_a.size(0)
    B = box_b.size(0)
    max_xy = tf.min(box_a[:, 2:].unsqueeze(1).expand(A, B, 2),
                       box_b[:, 2:].unsqueeze(0).expand(A, B, 2))
    min_xy = tf.max(box_a[:, :2].unsqueeze(1).expand(A, B, 2),
                       box_b[:, :2].unsqueeze(0).expand(A, B, 2))
    inter = tf.clamp((max_xy - min_xy), min=0)
...
def match(threshold, truths, priors, variances, labels, loc_t, conf_t, idx):
    ...
    N = (tf.sum(best_prior_overlap >= _th2) +
         tf.sum(best_prior_overlap >= _th3)) // 2
    ...
    if N > 0:
        N = tf.sum(stage2_overlap[:N]) if tf.sum( stage2_overlap[:N]) < N else N
        conf[stage2_idx[:N]] += 1
...
def decode(loc, priors, variances):
    ...
    boxXY = priors[:, :2] + loc[:, :2] * variances[0] * priors[:, 2:]   # bbox center
    boxWH = priors[:, 2:] * tf.math.exp(loc[:, 2:] * variances[1])      # bbox width and height
    boxLL = boxXY - boxWH * 0.5     # bbox low-left corner
    boxUR = boxLL + boxWH           # bbox upper-right coner
    boxes = tf.concat( (boxLL, boxUR), 1 )
    # boxes = tf.concat((
...
def log_sum_exp(x):
    ...
    """
    x_max = x.data.max()
    return tf.math.reduce_logsumexp(x - x_max, 1, keepdims=True) + x_max
...
def nms(boxes, scores, overlap=0.5, top_k=200):
    ...
    """
    selected_indices = tf.compat.v1.image.non_max_suppression( boxes, scores, top_k, overlap )
