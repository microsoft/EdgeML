...
int q15_fastgrnn_lr(Q15_T* const hiddenState, ITER_T hiddenDims,
  ...
  for (ITER_T t = 0; t < steps; t++) {
    ...
    if (normalize) {
      ...
      q15_v_hadamard(tparams->stdDev + offset * inputDims, tbuffers->normFeatures,
    ...
    q15_m_mulvec(tparams->W1, tbuffers->normFeatures, tparams->wRank, inputDims,
...
int q7xq15_q15_fastgrnn(Q15_T* const hiddenState, ITER_T hiddenDims,
  ...
  for (ITER_T t = 0; t < steps; t++) {
    ...
    if (normalize) {
      ...
      q7_v_hadamard(tparams->stdDev + offset * inputDims, tbuffers->normFeatures,
                    inputDims, tbuffers->normFeatures, tscales->stdDev,
                    tscales->normFeaturesHDStdDev);
    }
    else {
    ...
    // Process the new input and previous hidden state
    q15xq7_q15_m_mulvec(tparams->W, tbuffers->normFeatures, hiddenDims, inputDims,
