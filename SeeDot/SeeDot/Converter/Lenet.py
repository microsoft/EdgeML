...
class Lenet:
	...
	def readModel(self):
		self.Wc1 = readFileAsMat(os.path.join(getModelDir(), "Wc1"), "\t", float)
		self.Wc2 = readFileAsMat(os.path.join(getModelDir(), "Wc2"), "\t", float)
		self.Wf1 = readFileAsMat(os.path.join(getModelDir(), "Wf1"), "\t", float)
		self.Wf2 = readFileAsMat(os.path.join(getModelDir(), "Wf2"), "\t", float)
		#self.Wf3 = readFileAsMat(os.path.join(getModelDir(), "Wf3"), "\t", float)
		self.Bc1 = readFileAsMat(os.path.join(getModelDir(), "Bc1"), "\t", float)
		self.Bc2 = readFileAsMat(os.path.join(getModelDir(), "Bc2"), "\t", float)
		self.Bf1 = readFileAsMat(os.path.join(getModelDir(), "Bf1"), "\t", float)
		self.Bf2 = readFileAsMat(os.path.join(getModelDir(), "Bf2"), "\t", float)
...
class LenetFixed(Lenet):
	...
	def genInputForCompiler(self):
		with open(self.inputFile, 'w') as file:
			...
			file.write("let X2    = reshape(X, (1, 32, 32, 3), (1, 2))     in\n")
			...
			file.write("let Hc2PP = reshape(Hc2P, (1, 768), (1, 4, 2, 3))       in\n")
	...
	def writeModel(self):
		...
		writeMatAsArray(self.Bc1, 'Bc1', self.headerFile, shapeStr="[%d]" * 1 % (4))
		writeMatAsArray(self.Bc2, 'Bc2', self.headerFile, shapeStr="[%d]" * 1 % (12))
		writeMatAsArray(self.Bf1, 'Bf1', self.headerFile, shapeStr="[%d]" * 1 % (64))
		writeMatAsArray(self.Bf2, 'Bf2', self.headerFile, shapeStr="[%d]" * 1 % (10))
