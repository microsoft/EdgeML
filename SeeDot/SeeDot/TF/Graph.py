            ...
            while(tsCtnIdx + 1 < len(self.__tensorContentInput)):
                if (self.__tensorContentInput[tsCtnIdx] != '\\'):
                    ...
                    if (self.__tensorContentInput[tsCtnIdx+1] == 'n'):
                        ...
                        self.__tensorBytes[byteArrIdx] = (ord(self.__tensorContentInput[tsCtnIdx+1])-ord('0'))*64 + (ord(self.__tensorContentInput[tsCtnIdx+2])-ord('0'))*8 + (ord(self.__tensorContentInput[tsCtnIdx+3])-ord('0'))
                        tsCtnIdx += 3
                byteArrIdx += 1
                tsCtnIdx += 1
    ...
    def readFromFilePointer(self, fileP):
        ...
        while line:
            ...
            if (curToken == "node"):
                ...
                print("Versions node found. Ignoring remainder graph. Line =", cnt, file=sys.stderr)
                return True
            else:
