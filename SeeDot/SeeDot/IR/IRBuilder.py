...
class IRBuilder(ASTVisitor):
	...
	def __init__(self):
		...
		# internalVars: List of intermediate variables in the program whose type can be either float or int based on floating-point or fixed-point code generation respectively
	...
	def visitTanh(self, node:AST.Func):
		...
		if forFloat():
			...
			assert tanh_limit < np.iinfo(IR.DataType.getIntClass()).max, "tanh_limit in python fixed-point representation overflowed."
	...
	def visitSigmoid(self, node:AST.Func):
		...
		'''
		sigmoid_limit = int(np.ldexp(sigmoid_limit, -scale_in))
		assert sigmoid_limit < np.iinfo(IR.DataType.getIntClass()).max, "sigmoid_limit in python fixed-point representation overflowed."
		sigmoid_limit = IR.DataType.getInt(sigmoid_limit)
		sigmoid_limit = IR.Int(sigmoid_limit)
		'''
		...
		scale_new = self.getScale(max_val)
		...
		if debugCompiler():
			...
			print("\tinterval: %d, %d" % (self.varIntervals[expr_in.idf]))
			print("\tscale: %d" % (self.varScales[expr_in.idf]))
	...
	def visitLoop(self, node:AST.Loop):
		...
		#print("Scale of %s before reading profile is %d" % (idf, self.varScales[idf]))
		scale, intv = self.readProfileForMutableVars(idf)

		self.varScales[idf] = scale
		self.varIntervals[idf] = intv
		#print("Scale of %s after reading profile is %d" % (idf, self.varScales[idf]))

		(prog_in, expr_in) = self.visit(node.expr)

		start, end = node.start, node.end
		assert start == 1, "'loop' operator currently supports only iterations starting from 0."
