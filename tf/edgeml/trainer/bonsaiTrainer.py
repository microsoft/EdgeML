    ...
    def train(self, batchSize, totalEpochs, sess,
        ...
        for i in range(totalEpochs):
            ...
            if self.bonsaiObj.numClasses > 2:
                ...
                if(type==2):
                    _feed_dict = {self.X: np.vstack((Xtrain,Xtest)), self.Y: np.vstack((Ytrain,Ytest))}
                elif(type==1):
                    _feed_dict = {self.X: Xtest, self.Y: Ytest}
            ...
            if(i == totalEpochs-1):
                if(type==2):
                    ...
                    #pred = np.array(pred)
                    #pred[pred<0] = 0
                    print ("R2 : ",r2_score(Ytest,np.transpose(pred[0])[split:]))
                    result_dict["r2"] = r2_score(Ytest,np.transpose(pred[0])[split:])
                    print ("MAE : ",mean_absolute_error(Ytest,np.transpose(pred[0])[split:]))
                    result_dict["mae"] = mean_absolute_error(Ytest,np.transpose(pred[0])[split:])
                    print ("RMSE : ", np.sqrt(mean_squared_error(Ytest,np.transpose(pred[0])[split:])) )
                    result_dict["rmse"] = np.sqrt(mean_squared_error(Ytest,np.transpose(pred[0])[split:]))
		    print ("MAPE : ",np.mean(np.abs(Ytest - np.transpose(pred[0][split:]))*1.0 / Ytest))
                    print ("------------------------------------------")
		    ...
		    print ("MAPE : ",np.mean(np.abs(df.values[:,0]-df.values[:,1])/df.values[:,1]))
                    df.columns = ["Predictions","Truth"]
                    df.to_csv(currDir + "/" + "preds" + ".csv",index=False)


		    print ("Saving the predictions.")
		    #pd.DataFrame(pred[0]).to_csv(currDir + "/predictions.csv",index=False)
		    #pd.DataFrame(Ytest).to_csv(currDir + "/truth.csv",index=False)
