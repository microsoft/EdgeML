...
class NetworkJoint:
    ...
    def __createForwardGraph(self, X, initVarList):
        ...
        with tf.name_scope("final_output"):
            ret = tf.reshape(ret, [-1, self.numSubinstance, self.numTimeSteps, self.numOutput], name='bag_output')
            self.output = ret
    ...
    def createGraph(self, stepSize, alpha=0.0, beta=0.0, loss='smx',
        ...
        if self.initVarList is not None:
            ...
            k_ = graph.get_tensor_by_name('rnn/cell/kernel:0')
            b_ = graph.get_tensor_by_name('rnn/cell/bias:0')
