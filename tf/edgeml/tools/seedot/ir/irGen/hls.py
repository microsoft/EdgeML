...
from edgeml.tools.seedot.ir.irGen.irGenBase import IRGenBase
...
import edgeml.tools.seedot.ir.irGen.resourceEstimation as REst
...
class Hls(IRGenBase):
    ...
    def visitBopMulCir(self, node: AST.Bop1):
        ...
        if(typ_3.shape[0] > 1):
            cmdl_mul = IRUtil.loopNoUnroll(
                typ_3.shape, iters, [IR.Assn(expr_3_elt, rhs)])
        else:
            if(Res < Common.LUTUpperBound):
                cmdl_mul = IRUtil.loop(typ_3.shape, iters, [
                                       IR.Assn(expr_3_elt, rhs)], -1)
            else:
                cmdl_mul = IRUtil.loop(typ_3.shape, iters, [
                                       IR.Assn(expr_3_elt, rhs)], 0)
    ...
    def visitBopSparseMul(self, node: AST.Bop1):
        ...
        # ZX_t type for decls; notice that I use 'P' for changing shape to
        # [numWorkers, d]
        ZX_t_type = Type.Tensor([getNumWorkers(), P])
        ...
        cmd1a = IR.Assn(IRUtil.addIndex(
            expr_3, [memInd, IR.Int(0)]), IR.Int(0))
        cmd1 = IR.For(memInd, 0, IRUtil.lt(
            memInd, IR.Int(typ_3.size())), [cmd1a], fac=0)
        ...
        cmdv0 = IR.Pragmas(
            "#pragma HLS RESOURCE variable=Zx_t_ex core=ROM_1P_BRAM")
        ...
        cmdv5 = IRUtil.add(IRUtil.addIndex(
            expr_3, [i, IR.Int(0)]), IRUtil.addIndex(ZX_t_ex, [worker, i]))
        cmdv6 = IR.Assn(IRUtil.addIndex(expr_3, [i, IR.Int(0)]), cmdv5)
        loopAcc1 = IRUtil.loop([typ_3.size(), getNumWorkers()], [
                               i, worker], [cmdv6], 0)
    ...
    def visitBopSparseMulOld1(self, node: AST.Bop1):
        ...
        # add idx to tmp5 to paralellize
        cmd1a = IR.Assn(IRUtil.addIndex(ZX_t, [worker, i]), IR.Int(0))
        cmd1 = IRUtil.loop([getNumWorkers(), typ_3.size()],
                           [worker, i], [cmd1a], -1)
        ...
        for thread in range(0, getNumWorkers()):
            Aidx_t.append(
                IR.Var(expr_1.idf[0] + 'idx_t' + str(thread),  expr_1.idx, inputVar=True))
            Aval_t.append(
                IR.Var(expr_1.idf[0] + 'val_t' + str(thread), expr_1.idx, inputVar=True))
            X_t.append(
                IR.Var(expr_2.idf[0] + str(thread), expr_2.idx, inputVar=True))
        ...
        for thread in range(0, getNumWorkers()):
            cmd0b.append(
                IR.Assn(index_t[thread], IRUtil.addIndex(Aidx_t[thread], [job])))
            cmd0c.append(
                IR.Assn(offsetAssn[thread], IRUtil.addIndex(offset_t, [IR.Int(thread)])))
            cmd0d.append(IR.Assn(offsetAssn[thread], IRUtil.add(
                offsetAssn[thread], ite_t[thread])))
            cmd0e.append(IR.Assn(XAssn[thread], IRUtil.addIndex(
                X_t[thread], [offsetAssn[thread]])))
            cmd0f.append(
                IR.Assn(XAssn[thread],  IRUtil.shr(XAssn[thread], shr2)))  # shr
            cmd0g.append(
                IR.Assn(AvalAssn[thread], IRUtil.addIndex(Aval_t[thread], [job])))
            cmd0h.append(IR.Assn(AvalAssn[thread], IRUtil.shr(
                AvalAssn[thread], shr1)))  # shr
            cmd0i.append(IR.Assn(mulAssn[thread], IRUtil.mul(
                XAssn[thread], AvalAssn[thread])))
            cmd0j.append(IR.Assn(mulAssn[thread], IRUtil.shr(
                mulAssn[thread], H_1)))  # shr
            minus = IRUtil.sub(index_t[thread], IR.Int(1))
            cmd0k.append(IR.Assn(ZXAssn[thread], IRUtil.add(
                IRUtil.addIndex(ZX_t, [IR.Int(thread), minus]), mulAssn[thread])))
            cmd0l.append(IR.Assn(IRUtil.addIndex(
                ZX_t, [IR.Int(thread), minus]), ZXAssn[thread]))
            sum = IRUtil.add(ite_t[thread], IR.Int(1))
            cmd0m.append(IR.Assn(ite_t[thread], sum))
            cond = IRUtil.andd(IRUtil.cmp_neq(index_t[thread], IR.Int(
                0)),  IRUtil.cmp_lt(ite_t[thread], maxJobSize_t))
        ...
        cmd0 = IR.ForNoUnroll(job, 0, cmdt3, cmd0b + cmd0c + cmd0d +
                              cmd0e + cmd0f + cmd0g + cmd0h + cmd0i + cmd0j + cmd0k + cmd0n)

        cmdc1 = IR.Comment(" experimenting done with worker threads")

        prog = IR.Prog([cmdp0] + [cmdc0] + cmd1 +
                       cmd2 + cmd0a + [cmd0] + [cmdc1])
        prog_3 = IRUtil.concatPrograms(prog_1, prog_2, prog)
        ...
        cmd18 = IRUtil.add(IRUtil.addIndex(
            expr_3, [i, IR.Int(0)]), IRUtil.addIndex(ZX_t, [worker, i]))
        cmd19 = IR.Assn(IRUtil.addIndex(expr_3, [i, IR.Int(0)]), cmd18)
        loopAcc = IRUtil.loopUnroll(
            [typ_3.size(), getNumWorkers()], [i, worker], [cmd19])
        cmdp1 = IR.Pragmas("#else")
        prog_4 = IR.Prog(loopAcc + [cmdp1])

        cmdv0 = IR.Pragmas(
            "#pragma HLS RESOURCE variable=SpMV_t core=ROM_1P_BRAM")
        # memset ZX using for-loops
        cmdv1 = IR.Assn(IRUtil.addIndex(expr_3, [i, IR.Int(0)]), IR.Int(0))
        ...
        cmdv5 = IRUtil.add(IRUtil.addIndex(
            expr_3, [i, IR.Int(0)]), IRUtil.addIndex(ZX_t_ex, [worker, i]))
        cmdv6 = IR.Assn(IRUtil.addIndex(expr_3, [i, IR.Int(0)]), cmdv5)
        loopAcc1 = IRUtil.loopUnroll(
            [typ_3.size(), getNumWorkers()], [i, worker], [cmdv6])
    ...
    def visitBopSparseMulOld(self, node: AST.Bop1):
        ...
        # cmdl_{shr1, shr2, mul}
        cmd12 = IR.Assn(x, IRUtil.shr(
            IRUtil.addIndex(expr_1, [Aval_idx]), shr1))
        ...
        loop11 = [IR.While(IRUtil.cmp_gte(j, IRUtil.zero), [
    ...
    def visitSum(self, node: AST.Sum):
        ...
        memInd = self.getTempIterator()
        cmd1a = IR.Assn(IRUtil.addIndex(
            expr_2, [memInd, IR.Int(0)]), IR.Int(0))
        ...
        cmd_l1 = IRUtil.loop([i_ed - i_st], [i_iter], prog_1.cmd_l +
                             sum_loop + [IR.Assn(i_var, IRUtil.inc(i_var))], fac)
