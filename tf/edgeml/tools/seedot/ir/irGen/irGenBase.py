...
class IRGenBase(ASTVisitor):
    ...
    def compile_sum(self, N: int, H_1: int, H_2: int, src: IR.Var, dst: IR.Var, idx_to_prefix: bool):
        ...
        def get_cmdl_body(shr_n: int) -> IR.CmdList:
            ...
            src_2i = IRUtil.addIndex(
                src, [IRUtil.mul(IR.Int(2), i)], idx_to_prefix)
            src_2i1 = IRUtil.addIndex(
                src, [IRUtil.inc(IRUtil.mul(IR.Int(2), i))], idx_to_prefix)
    ...
    def sparseSum(self, N: IR.Var, H_1: int, H_2: int, src: IR.Var, dst: IR.Var, idx_to_prefix: bool):
        ...
        def get_cmdl_body(shr_n: int) -> IR.CmdList:
            ...
            res = \
                ...
                [IR.Assn(n_cur, IRUtil.shrUint(
                    IRUtil.inc(n_cur), 1))] + shrCmd
            return res
        ...
        cmdl_res = \
            ...
            IRUtil.loop([H_1], [h], get_cmdl_body(1)) + \
            IRUtil.loop([H_2], [h], get_cmdl_body(0)) + \
            [IR.Assn(dst, IRUtil.shrVar(IRUtil.addIndex(
                src, [IRUtil.zero], idx_to_prefix), shr))]
        decls_res = {n_cur.idf: Type.Int(),
                     nHalf.idf: Type.Int(),
                     itmp .idf: Type.Int(),
                     shr.idf: Type.Int(),
    ...
    def _get_exp_expr(self, e_pos: IR.Expr, table: IR.Var):  # -> (IR.CmdList, IR.Expr)
        ...
        for i in range(idx1_num):
            ...
            rhs = IRUtil.add_idx_priv(IRUtil.addIndex(
                table, [idx1]), idx2_tmp, self.expB)
        ...
        expr_res = [IRUtil.addIndex(table_tmp, [IR.Int(i)])
                    for i in range(idx1_num)]
    ...
    def compile_exp(self, e: IR.Expr, p: int):  # -> IR.CmdList * IR.Expr * decl dict
        ...
        if True:
            ...
            (cmdl_exp_neg, expr_exp_neg, decls_exp_neg) = self._get_exp_expr(
                IRUtil.negate(e), table_neg)
    ...
    def visitId(self, node: AST.ID):
        ...
        expr_1 = IR.Var(
            idf, inputVar=True if idf in self.VAR_IDF_INIT else False)
    ...
    def visitReshape(self, node: AST.Reshape):
        ...
        if True:
            assert((typ_1.dim == 1) or (
                typ_2.dim == 2 and typ_2.shape[0] == 1))
        ...
        # private values are used in array index).
            ...
            for i in reversed(range(1, typ_1.dim)):
                ...
                cmdl_inc = cmdl_inc + \
                    [IR.Assn(carry, IR.CExpr(IRUtil.eq(itr_cur, sz_cur), IRUtil.one, IRUtil.zero)),
                     IR.Assn(itr_cur, IR.CExpr(
                         IRUtil.eq(carry, IRUtil.one), IRUtil.zero, itr_cur)),
            ...
            expts_2 = copy_dict(expts_1, {expr_2.idf:    p_2})
            intvs_2 = copy_dict(intvs_1, {expr_2.idf: intv_2})
            decls_2.update(dict((var.idf, Type.Int())
                                for var in tuple([carry] + iters_1)))
    ...
    def visitMaxpool(self, node: AST.Maxpool):
        ...
        #       after Relu)
        if True:
            rhs = IRUtil.max_uint(tmp, IRUtil.addIndex(
                expr_1, idx_1(jd, kd)))  # if expr_1[..] is always >=0
        else:
            rhs = IRUtil.max_sint(tmp, IRUtil.addIndex(
                expr_1, idx_1(jd, kd)))  # if expr_1[..] can be <0
        cmdl_max = \
    ...
    def visitIndex(self, node: AST.Index):
        ...
        (prog_2, expr_2,  decls_2, expts_2,
         intvs_2, cnsts_2) = self.visit(node.index)
...
#        expr_3 = IR.add_idx_str(expr_1, '[%s]' % out.getvalue())
    ...
    def visitUop(self, node: AST.Uop):   # {+,-}
        ...
        # e: Tensor(), or Tensor(..)
            ...
            cmdl_assn = IRUtil.loop(typ_2.shape, iters, [
                                    IR.Assn(expr_2_elt, rhs)])
    ...
    def visitBopMul1DTensor(self, node: AST.Bop1):
        ...
        # cmdl_{shr1, shr2, mul}
        cmdl_shr1 = IRUtil.loop_shr(
            expr_1_shr, expr_1, typ_1.shape, iters_1, shr1)
        cmdl_shr2 = IRUtil.loop_shr(
            expr_2_shr, expr_2, typ_2.shape, iters_2, shr2)
        ...
        cmdl_mul = IRUtil.loop(typ_3.shape, iters_3, [
                               IR.Assn(expr_3_elt, rhs)])
    ...
    def visitBopMul2DTensor(self, node: AST.Bop1):
        ...
        rhs = IRUtil.mul(expr_1_shr_elt, expr_2_shr_elt)
        cmdl_mul = IRUtil.loop([I, K, J], [i, k, j], [
                               IR.Assn(expr_mul_elt, rhs)])
    ...
    def visitBopMulCir(self, node: AST.Bop1):
        ...
        # cmdl_*
        cmdl_shr1 = IRUtil.loop_shr(
            expr_1_shr, expr_1, typ_3.shape, iters, shr1)
        cmdl_shr2 = IRUtil.loop_shr(
            expr_2_shr, expr_2, typ_3.shape, iters, shr2)
    ...
    def visitBopConv(self, node: AST.Bop1):
        ...
        cmdl_shr = \
            ...
            IRUtil.loop_shr(expr_2_shr, expr_2, [HF, WF, CI, CO], [
                            hf, wf, ci, co], shr2)
        ...
        # cmdl_mul
        expr_1_pad_elt = IRUtil.addIndex(
            expr_1_pad, [n, IRUtil.add(h, hf), IRUtil.add(w, wf), ci])
        ...
        decls_3.update({expr_1_shr.idf: typ_1,
                        ...
                        expr_2_shr.idf: typ_2,
                        expr_mul  .idf: typ_mul})
        decls_3.update(dict((var.idf, Type.Int())
                            for var in [h_tmp, w_tmp, cnt]))
        decls_3.update(decls_sum)
    ...
    def visitBop2(self, node: AST.Bop2):  # {+, -}
        ...
        # e : Tensor(), or Tensor(..)
            ...
            if expr_2.idf == 'Min':
                iters = self.getTempIterators(typ_3.dim)
                p = IRUtil.print_loop(typ_3.shape, iters, [
                                      IR.PrintAsFloat(IRUtil.addIndex(expr_3, iters), p_3)])
                p = []
            else:
                p = []
    ...
    def visitExp(self, node: AST.Func):
        ...
        if True:
            p_2 = self.get_expnt_exp(p_1, 'neg')  # expr_1 is always <0
        else:
            p_2 = self.get_expnt_exp(p_1, 'pos')  # expr_1 can be >=0 or <0
    ...
    def visitArgMax(self, node: AST.Func):
        ...
        cmdl_assn = IRUtil.loop(typ_1.shape, iters,
                                [IR.Assn(newmax, IRUtil.max_sint(maxval, expr_1_elt)),
                                 IR.Assn(maxind, IR.CExpr(
                                     IRUtil.eq(maxval, newmax), maxind, curind)),
        ...
        decls_2 = copy_dict(decls_1, dict((var.idf, Type.Int())
                                          for var in [curind, maxind, maxval, newmax]))
        expts_2 = expts_1
        intvs_2 = intvs_1
    ...
    def visitSgn(self, node: AST.Func):
        ...
        expr_2 = IRUtil.cond_zero(expr_1_elt, IRUtil.one, IRUtil.zero)
        decls_2 = decls_1
        expts_2 = expts_1
        intvs_2 = intvs_1
    ...
    def visitCond(self, node: AST.Cond):
        ...
        (prog_2, expr_2,  decls_2, expts_2, intvs_2,
         cnsts_2) = self.visit(node.trueBlock)
        self.set_arg2(node.falseBlock, decls_2, expts_2, intvs_2, cnsts_2)
        (prog_3, expr_3,  decls_3, expts_3, intvs_3,
         cnsts_3) = self.visit(node.falseBlock)
        ...
        # e2,e3 : Tensor(), or Tensor(..)
            ...
            (m_3, M_3) = intv_3
            if p_2 >= p_3:
                (shr_n_2, shr_n_3) = (0, p_2 - p_3)
            else:
                (shr_n_2, shr_n_3) = (p_3 - p_2, 0)
            p_4 = max(p_2, p_3)
    ...
    def visitLet(self, node: AST.Let):
        ...
        if Type.isInt(typ_1):
            ...
            (prog_2, expr_2,  decls_2, expts_2,
             intvs_2, cnsts_2) = self.visit(node.expr)
    ...
    def printVarDecls(self, writer, decls: dict):
        for decl in decls:
            ...
            if Type.isInt(type):
                ...
                shape_str = ''.join(['[' + str(n) + ']' for n in typeshape])
            writer.printf('%s %s%s;\n', typ_str, idf_str,
