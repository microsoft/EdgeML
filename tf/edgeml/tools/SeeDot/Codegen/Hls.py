...
class Hls(CodegenBase):
    ...
    def printHlsHeader(self):
        ...
        self.out.printf('#pragma HLS ARRAY_PARTITION variable=ZX_t_ex block factor=' +
                        str(getNumWorkers()) + ' dim=1 \n')
        self.out.increaseIndent()
        # self.out.printf('#pragma HLS INTERFACE ap_ctrl_none port=return \n ',
        # indent=True)

        # to account for the constraint of 90 DSPs on device
        self.out.printf(
            '#pragma HLS ALLOCATION instances=mul limit=75 operation \n ', indent=True)
        #printf('char buff[8];\n\n', indent=True)
    ...
    def printMemset(self, ir):
        self.out.printf(
            '//memset cant be used in HLS code; using for-loop instead\n', indent=True)
        if(self.dim == 1):
            self.out.printf(
                'for(int i=0;i<%d;i++){ \n #pragma HLS UNROLL \n', ir.len, indent=True)
            self.out.increaseIndent()
